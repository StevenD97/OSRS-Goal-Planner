...
export function FarmRunDetailPage() {
  ...
  if (!run) {
    return (
      <div>
        <p className="text-muted">Unknown farm run.</p>
        <Link to="/farm-runs" className="text-accent hover:underline">
          Back to Farm Runs
        </Link>
      </div>
    );
  ...
  return (
    <div>
      <Link to="/farm-runs" className="text-sm text-accent hover:underline">
        &larr; All farm runs
      </Link>

      <div className="mt-2 flex flex-wrap items-start justify-between gap-3">
        <div>
          <h1 className="text-2xl font-semibold text-ink">{run.name}</h1>
          <p className="mt-1 max-w-2xl text-sm text-muted">{run.description}</p>
        </div>
        <button
          onClick={() => resetRun(run.id)}
          className="flex-none rounded-row border border-line-strong px-3 py-1.5 text-sm text-ink-2 hover:bg-surface-2"
        >
          Reset checklist
        </button>
      </div>
      ...
      {run.requirements && run.requirements.length > 0 && (
        <ul className="mt-3 flex flex-wrap gap-2">
          {run.requirements.map((req) => (
            <li
              key={req}
              className="rounded-full bg-surface-2 px-3 py-1 text-xs text-ink-2"
            >
              {req}
            </li>
          ))}
